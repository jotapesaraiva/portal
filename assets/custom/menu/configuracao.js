...
$(document).ready(function() {
        ...
        $.ajax({
            url : "https://producaoh.sefa.pa.gov.br/portal/menu/meu_perfil/listar_usuarios",
            type: "GET",
            dataType: "JSON",
            success: function(data) {
                ...
                if(data.usuario.id_permissao == '1') {
                    $('#user').bootstrapSwitch('state', true);
                } if(data.usuario.id_permissao == '2') {
                    $('#admin').bootstrapSwitch('state', true);
                }else {
                    $('#super').bootstrapSwitch('state', true);
                }
                $('[name="cargo"]').val(data.usuario.nome_cargo);
                $('[name="equipe"]').val(data.usuario.nome_grupo);
