...
$(document).ready(function() {
    table = $('#table').DataTable({
      ...
      "ajax": {
          url : server+"/tarefas_list",//json datasource
...
function add_tarefas() {
    ...
    $('#modal_tarefas').modal('show'); // show bootstrap modal
    $('.modal-title').text('Adicionar tarefas'); // Set Title to Bootstrap modal title
}

function edit_tarefas(id) {
    save_method = 'update';
    $('#form')[0].reset(); // reset form on modals
    $(".selectpicker").val('').selectpicker('refresh'); //reset selectcpicker
    $('.form-group').removeClass('has-error'); // clear error class
    ...
    $.ajax({
        url : server+"/tarefas_edit/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data) {
            $('[name="id"]').val(data.id);
            $('[name="nome"]').val(data.nome_tarefas);
            $('[name="mensagem"]').val(data.mensagem_tarefas);
            $('[name="data_inicio"]').val(data.data_inicio_tarefas);
            $('[name="data_fim"]').val(data.data_fim_tarefas);
            $('[name="grupo"]').selectpicker('val', data.id_grupo);
            $('[name="mantis_solicitado"]').val(data.mantis_solicitado);
            $('[name="mantis"]').val(data.mantis);
            //
            $('#modal_tarefas').modal('show'); // show bootstrap modal when complete loaded
            $('.modal-title').text('Editar tarefas'); // Set title to Bootstrap modal title
        },
...
function save(){
    ...
    if(save_method == 'add') {
        ...
        url = server+"/tarefas_add";
    } else {
        url = server+"/tarefas_update";
    }
...
function delete_tarefas(id){
    if(confirm('Você tem certeza que quer deletar o tarefas ?')) {
        // ajax delete data to database
        $.ajax({
            url : server+"/tarefas_delete/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data) {
