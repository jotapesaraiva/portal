...
class Breadcrumbs {
	...
	function unshift($page, $href){
		...
		// add at firts
		array_unshift($this->breadcrumbs, array('page' => $page. $this->crumb_divider, 'href' => $href));
	}
	...
	function show(){
		if ($this->breadcrumbs) {
			...
			foreach ($this->breadcrumbs as $key => $crumb) {
				...
				if (end($keys) == $key) {
					$output .= $this->crumb_last_open.'<a href="' . $crumb['href'] . '">' . $crumb['page'] . '</a> '.$this->crumb_close;
				} else {
