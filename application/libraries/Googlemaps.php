...
class Googlemaps {
	...
	function create_map(){
		...
		if ($this->loadAsynchronously) {
			...
			// google.maps.event.addListener(window, "load", initialize_'.$this->map_name.');
			window.onload=initialize_'.$this->map_name.';
			';
		}

		if($this->refreshMarkers!=""){
			$this->output_js_contents .= '
			setTimeout("location.reload(true);",'.$this->refreshMarkers.');';
		}
...
//#####################################################################################################################################################
		...
		if ($this->maps_loaded == 0){
			...
			{
				if ($this->https) { $apiLocation = 'https://maps'; }else{ $apiLocation = 'http://maps'; }
